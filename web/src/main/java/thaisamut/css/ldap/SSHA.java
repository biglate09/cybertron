...
public class SSHA {
	...
	// in java 8 the code must be modify to java.util.Base64.getEncoder().encodeToString(byte[] bytes);
	private static BASE64Encoder enc = new BASE64Encoder();
	// in java 8 the code must be modify to java.util.Base64.getDecoder().decode(String cypto);
	private static BASE64Decoder dec = new BASE64Decoder();
	//see more info : http://stackoverflow.com/questions/36578625/base64encoder-is-internal-api-and-may-be-removed-in-future-release
...
//		String label = "{SSHA}";
		...
		if (verbose) {
			...
			System.out.println("binary digest of password plus binary salt: "
					+ pwhash + salt);
			System.out
					.println("Now we base64 encode what is respresented above this line ...");
		}
	...
	private boolean checkDigest0(String digest, String entity) {
		...
		if (verbose) {
			...
			System.out
					.println("Salted Hash presented in hex: " + toHex(pwhash));
		}
